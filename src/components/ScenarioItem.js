...
function ScenarioItem(props) {
    ...
    const deleteScenario = () => {
        axios.delete(`https://http-localhost-3001.onrender.com/scenarios/`+props.scenario.id)
        .then(() => {
            //delete the child vehicles
            let urls = props.scenario.vehicles.map((id) => {
                return `https://http-localhost-3001.onrender.com/vehicles/${id}`;
            });
    ...
    const editScenario = (data) => {
        ...
        if(data.scenarioName !== prevName || data.scenarioTime !== prevTime) {
            axios
            .patch(`https://http-localhost-3001.onrender.com/scenarios/${props.scenario.id}`, {
