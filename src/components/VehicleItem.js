...
function VehicleItem(props) {
    ...
    const editVehicle = (data) => {
        if(data.vehicleName !== namePrev || data.vPositionX !== positionXPrev || data.vPositionY !== positionYPrev || data.vDirection !== directionPrev || data.vSpeed !== speedPrev) {
            axios
            .patch(`https://http-localhost-3001.onrender.com/vehicles/${props.vehicle.id}`, {
    ...
    const confirmDelete = () => {
      ...
      const deleteHandler = () => {
        axios.delete(`https://http-localhost-3001.onrender.com/vehicles/`+props.vehicle.id)
          .then((response) => {
            //update parent scenario
            axios.get(`https://http-localhost-3001.onrender.com/scenarios/${props.vehicle.scenarioId}`).then((res) => {
                ...
                console.log(temp);
                axios
                .patch(`https://http-localhost-3001.onrender.com/scenarios/${props.vehicle.scenarioId}`, {
                    vehicles : temp
                }).then(() => {
